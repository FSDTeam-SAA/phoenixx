...
const BlogPostForm = ({
  ...
  const { data: subcategoryData, isLoading: isSubcategoriesLoading } =
    useSubCategoriesQuery(category, {
      skip: !category,
  ...
  const truncateTo1000Words = useCallback((html) => {
    ...
    const processTextNode = (textNode) => {
      ...
      const nodeWords = normalizedNodeText
        .split(" ")
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: `focus:outline-none p-4 min-h-[240px] max-h-[240px] overflow-y-auto ${
  ...
  const beforeUpload = (file) => {
    ...
    if (fileSizeInMB > 500) {
      message.info(
        `Uploading large files (${fileSizeInMB.toFixed(2)} MB). Please wait...`
      );
    }
    return false;
  };
      ...
      if (isEditing && postId) {
        ...
        initialImages.forEach((initialImage) => {
          const stillExists = fileList.some(
            (file) =>
              file.path === initialImage.path || file.url === initialImage.url
          );
          if (!stillExists) {
    ...
    <>
      ...
      <div
        className={`min-h-screen ${
          isEditing ? "" : "py-4 sm:py-8 px-2 sm:px-4"
        ...
        <div className="max-w-4xl mx-auto">
          <Card
            className={`rounded-xl shadow-lg border-0 overflow-hidden ${
              isEditing ? "border-0 shadow-none" : ""
            } ${
              isDarkMode
          ...
          >
            {!isEditing && (
              <div className="text-white p-5 bg-[#0000FF] rounded-xl">
                {/* <Image
            ...
            <div className={`${!isEditing && "py-4 sm:p-6"}`}>
                ...
                <Row gutter={[16, 16]}>
                  <Col xs={24} md={12}>
                    ...
                    <Select
                      ...
                      className={`w-full ${
                        isDarkMode ? "ant-select-dark" : ""
                      } ${
                        formErrors.category
                          ? "border-red-500 ant-select-status-error"
                          : ""
                      }`}
                      size={isMobile ? "middle" : "large"}
                      options={[...categoryOptions].reverse()}
              ...
              <div className="mb-6 sm:mb-8">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div
                    className={`text-sm font-medium word-count-indicator ${
                      wordCount > 900
                        ? "word-count-error"
                        : wordCount > 800
                        ? "word-count-warning"
                ...
                >
                  {/* Toolbar */}
                  <div
                    className={`flex gap-1 px-1 py-2 border-b ${
                      isDarkMode
              ...
              <div className="mb-6 sm:mb-8">
                ...
                >
                  Featured Images{" "}
                  <span className="text-xs font-normal">(Maximum 3)</span>
                </Title>
                <Card
                  className={`border-2 border-dashed rounded-xl hover:border-blue-400 transition-all text-center cursor-pointer ${
                    isDarkMode
